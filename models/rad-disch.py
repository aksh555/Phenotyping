...
def main():
    ...
    if args.do_eval:
        model.load_state_dict(torch.load(
            os.path.join(model_dir, 'pheno_rad_disch.bin')))
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, 'pheno_rad_disch.txt'), 'wb'))  # save report
        print(clf_report)
        with open(os.path.join(out_dir, 'pheno_rad_disch.txt'), 'w') as f:
            f.write(outs)
