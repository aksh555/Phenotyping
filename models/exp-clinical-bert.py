...
class BERT(BaseModel):
    ...
    def forward(self, x, x1, attention_mask, attention_mask1, labels=None):
        ...
        attn_output = torch.cat((attn_output1,attn_output), dim=2)
...
def main():
        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_loss:
                torch.save(model.state_dict(), os.path.join(
                    model_dir, 'clinical-bert-2notes-conv.bin'))
                best_loss = val_loss
                stop_it = 0
            else:
                stop_it += 1
    ...
    if args.do_eval:
        model.load_state_dict(torch.load(
            os.path.join(model_dir, 'clinical-bert-2notes-conv.bin')))
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, 'classification_report_clinical-bert-2notes.txt'), 'wb'))  # save report
        print(clf_report)
        with open(os.path.join(out_dir, 'res-clinical-bert-2notes.txt'), 'w') as f:
            f.write(outs)
