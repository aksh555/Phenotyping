...
class BERT(BaseModel):
    def __init__(self, args, numClasses, embed_file=None, num_filter_maps=256, embed_size=128, dropout=0.2, vocab_size=500):
        ...
        self.conv1 = nn.Conv1d(2*self.rnn_hidden_size,2*self.rnn_hidden_size,kernel_size=3, padding=1)
        
        # self.convlist = nn.ModuleList([nn.Conv1d(self.embed_size, self.embed_size, kernel_size=3, padding=1) for i in range(numClasses)])
        ...
        self.final = nn.Linear(4*self.rnn_hidden_size, self.numClasses)
        ...
        self.lin1 = nn.Linear(2*self.rnn_hidden_size, self.rnn_hidden_size*2)
        self.lin2 = nn.Linear(self.embed_size, self.rnn_hidden_size*2)
    ...
    def forward(self, x, attention_mask, labels=None):
        ...
        last_hidden = self.dropout(outputs)
        # print('lh', last_hidden.shape)


        conv_output1 = self.conv1(last_hidden.permute(0,2,1))
        conv_output1 = self.avgpool(conv_output1)
        conv_output1 = conv_output1.permute(0,2,1)
        # print('c1', conv_output1.shape)

        # conv_output1 = self.lin1(conv_output1)
        conv_output1 = torch.mean(conv_output1, dim=1)
        # print('c1', conv_output1.shape)

        # # SKIP
        # # print(attn_output.shape)
        conv_output2 = attn_output.permute(0,2,1)
        conv_output2 = self.conv2(conv_output2)
        conv_output2 = self.avgpool(conv_output2)
        conv_output2 = conv_output2.permute(0,2,1)
        # print('c2', conv_output2.shape)

        conv_output2 = self.lin2(conv_output2)
        conv_output2 = self.act(conv_output2)
        conv_output2 = torch.mean(conv_output2, dim=1)
        print('c2', conv_output2.shape)
        
        op = torch.cat((conv_output1, conv_output2), dim=1)
...
def main():
    ...
    tz = BertTokenizer.from_pretrained(args.model, padding=True, truncation=True)
    ...
    #loss_fn = nn.MultiLabelSoftMarginLoss(weight=torch.FloatTensor(wts))
        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_loss:
                torch.save(model.state_dict(), os.path.join(
                    model_dir, f'rnn-conv-disch-ref-back.bin'))
                best_loss = val_loss
                stop_it = 0
            else:
                stop_it += 1
    ...
    if args.do_eval:
        model.load_state_dict(torch.load(
            os.path.join(model_dir, f'rnn-conv-disch-ref-back.bin')))
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, f'CR_rnn-conv-disch-ref-back.txt'), 'wb'))  # save report
        print(clf_report)
        with open(os.path.join(out_dir, f'RES-rnn-conv-disch-ref-back.txt'), 'w') as f:
            f.write(outs)
