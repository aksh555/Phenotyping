...
def main():
    ...
    df = pd.read_csv(os.path.join(files_dir, 'pheno-nurse-rad.csv'))
    ...
    if args.do_eval:
        model.load_state_dict(torch.load(
            os.path.join(model_dir, 'pheno-rad-nurse.bin')))
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, 'pheno-rad-nurse.txt'), 'wb'))  # save report
        print(clf_report)
        with open(os.path.join(out_dir, 'pheno-rad-nurse.txt'), 'w') as f:
            f.write(outs)
