...
class BERTLATA(BaseModel):
    ...
    def forward(self, x, attention_mask, labels=None):
        ...
        attn_output, attn_output_weights = self.mheadattn(
            outputs, outputs, outputs, key_padding_mask=attention_mask)
...
def main():
    ...
    df = pd.read_csv(os.path.join(files_dir, 'disch.csv'))
    tz = BertTokenizer.from_pretrained(
        './disch_tokenizer', padding=True, truncation=True)
    model = BERTLATA(numClasses, embed_size=512, vocab_size=tz.vocab_size)
        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_loss:
                torch.save(model.state_dict(), os.path.join(
                    model_dir, 'bertattn_best-icd-bce.bin'))
                best_loss = val_loss

    if args.do_eval:
        model.load_state_dict(torch.load(
            os.path.join(model_dir, 'bertattn_best-icd-bce.bin')))
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, 'classification_report_-bce.txt'), 'wb'))  # save report
        # print(clf_report)
        with open(os.path.join(out_dir, 'bert-pheno-bce.txt'), 'w') as f:
            f.write(outs)
