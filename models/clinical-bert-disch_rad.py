...
class BERT(BaseModel):
    def __init__(self, numClasses, embed_file=None, num_filter_maps=256, embed_size=128, dropout=0.2, vocab_size=500):
        ...
        self.final = nn.Linear(100, self.numClasses)
        self.mheadattn = nn.MultiheadAttention(self.embed_size, 4)
        self.m = nn.ReLU()
        self.conv = nn.Conv1d(2*768,10,kernel_size=3, padding=1)
        # self.conv2 = nn.Conv1d(200,10,kernel_size=3, padding=1)
    ...
    def forward(self, x, x1, attention_mask, attention_mask1, labels=None):
        ...
        with torch.no_grad():
            ...
            outputs1 = self.encoder(input_ids=x1, attention_mask=attention_mask1)
        ...
        attn_output, attn_output_weights = self.mheadattn(outputs1, outputs, outputs, key_padding_mask=attention_mask1)
        # attn_output1, attn_output_weights = self.mheadattn(outputs1, outputs1, outputs1, key_padding_mask=attention_mask1)
        ...
        attn_output = torch.cat((outputs,attn_output), dim=2)
...
def get_feats(model, train_data_loader, valid_data_loader, test_data_loader):
    ...
    df_tr.to_csv("./dump_files/train_pheno_disch_feats-clinicalbert.csv")
    df_val.to_csv("./dump_files/valid_pheno_disch_feats-clinicalbert.csv")
    df_test.to_csv("./dump_files/test_pheno_disch_feats-clinicalbert.csv")
...
def main():
    ...
    df = pd.read_csv(os.path.join(files_dir, 'pheno_disch_rad-ref.csv'))
    tz = BertTokenizer.from_pretrained(
        "emilyalsentzer/Bio_ClinicalBERT", padding=True, truncation=True)
    model = BERT(numClasses, embed_size=768, vocab_size=tz.vocab_size, num_filter_maps=3072)
        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_loss:
                torch.save(model.state_dict(), os.path.join(
                    model_dir, 'clinical-bert-cross-2notes-conv.bin'))
                best_loss = val_loss
                stop_it = 0
            else:
                stop_it += 1
    ...
    if args.do_eval:
        model.load_state_dict(torch.load(
            os.path.join(model_dir, 'clinical-bert-cross-2notes-conv.bin')))
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, 'classification_report_clinical-bert-cross-2notes.txt'), 'wb'))  # save report
        print(clf_report)
        with open(os.path.join(out_dir, 'res-clinical-bert-cross-2notes.txt'), 'w') as f:
            f.write(outs)
