...
class BERT(BaseModel):
    def __init__(self, numClasses, embed_file=None, num_filter_maps=256, embed_size=128, dropout=0.2, vocab_size=500):
        ...
        self.mheadattn = nn.MultiheadAttention(self.embed_size, 8)
    ...
    def forward(self, x, x1, attention_mask, attention_mask1, labels=None):
        ...
        attn_output, attn_output_weights = self.mheadattn(
            outputs, outputs1, outputs1, key_padding_mask=attention_mask1)
...
class ENCAML(BaseModel):
    def __init__(self, numClasses, embed_file=None, num_filter_maps=512, embed_size=50, dropout=0.2, vocab_size=500):
        ...
        self.final = nn.Linear(len(kernel_size)*self.embed_size, self.numClasses)
        self.mheadattn = nn.MultiheadAttention(self.embed_size, 8)

    def forward(self, x, x1, attention_mask, attention_mask1):
        x = self.embed(x)
        x = self.dropout(x)
        x = x.transpose(1, 2)
        m = []
        ...
        for i, conv in enumerate(self.conv):
            ...
            #print('x1', x1.shape)
            attn_output, attn_output_weights = self.mheadattn(
            x1, x1, x1, key_padding_mask=attention_mask)
            attn_output = attn_output.permute(1, 0, 2)
            m.append(attn_output)
        m = torch.cat(m, -1)
        #print('m', m.shape)
        yhat = self.final(m)
        
        yhat = torch.mean(yhat, dim=1)
        #print('yhat', yhat.shape)
        return yhat, attn_output_weights
...
def main():
    ...
    df = pd.read_csv(os.path.join(files_dir, 'pheno_disch_rad.csv'))
    tz = BertTokenizer.from_pretrained(
        './disch_rad_notes_tokenizer', padding=True, truncation=True)
    model = ENCAML(numClasses, embed_size=512, vocab_size=tz.vocab_size)
        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_loss:
                torch.save(model.state_dict(), os.path.join(
                    model_dir, 'encattn_pheno-bce.bin'))
                best_loss = val_loss

    if args.do_eval:
        model.load_state_dict(torch.load(
            os.path.join(model_dir, 'encattn_pheno-bce.bin')))
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, 'classification_report_pheno3-bce.txt'), 'wb'))  # save report
        # print(clf_report)
        with open(os.path.join(out_dir, 'bert3-pheno-bce.txt'), 'w') as f:
            f.write(outs)
