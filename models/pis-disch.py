...
def main():
    ...
    df = pd.read_csv(os.path.join(files_dir, 'pheno-disch-phys.csv'))
        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_loss:
                torch.save(model.state_dict(), os.path.join(
                    model_dir, 'pheno-phys-disch.bin'))
                best_loss = val_loss
                stop_it=0
            else:
                stop_it += 1
    ...
    if args.do_eval:
        model.load_state_dict(torch.load(
            os.path.join(model_dir, 'pheno-phys-disch.bin')))
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, 'pheno-phys-disch.txt'), 'wb'))  # save report
        print(clf_report)
        with open(os.path.join(out_dir, 'pheno-phys-disch.txt'), 'w') as f:
            f.write(outs)
