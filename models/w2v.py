...
class BERT(BaseModel):
    def __init__(self, args, numClasses, embed_file=None, num_filter_maps=256, embed_size=128, dropout=0.2, vocab_size=500):
        ...
        self.attn1 = nn.MultiheadAttention(self.embed_size, 6)
        self.act = nn.ReLU()
    ...
    def forward(self, x, attention_mask, labels=None):
        ...
        attn_output = torch.add(skip1,attn_output)
        ...
        key = attn_output.permute(2,0,1)
        query = skip_initial.permute(1,0,2)
        # print('key',key.shape)
        # print('query',query.shape)
        attn_output,_ = self.attn1(query,key,query)
