...
class BERT(BaseModel):
    ...
    def forward(self, x, attention_mask, labels=None):
        ...
        attn_output = outputs.permute(1,2,0)
        ...
        # attn_output, attn_output_weights = self.mheadattn(attn_output, attn_output, attn_output)
        # attn_output = attn_output.permute(1,0,2)
...
def main():
        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_loss:
                torch.save(model.state_dict(), os.path.join(
                    model_dir, f'{args.model_name}-conv-disch-ref-back.bin'))
                best_loss = val_loss
                stop_it = 0
            else:
                stop_it += 1
    ...
    if args.do_eval:
        model.load_state_dict(torch.load(
            os.path.join(model_dir, f'{args.model_name}-conv-disch-ref-back.bin')))
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, f'CR_{args.model_name}-conv-disch-ref-back.txt'), 'wb'))  # save report
        print(clf_report)
        with open(os.path.join(out_dir, f'RES-{args.model_name}-conv-disch-ref-back.txt'), 'w') as f:
            f.write(outs)
