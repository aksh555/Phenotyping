...
import gensim
#w2v_model = gensim.models.KeyedVectors.load_word2vec_format('./models/w2v-disch-100-5-3.bin', binary=True)
...
def main():
        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_loss:
                torch.save(model.state_dict(), os.path.join(
                    model_dir, 'Enc_best-pheno-softmargin.bin'))
                best_loss = val_loss
    ...
    if args.do_eval:
        ...
        # get features
        get_feats(model, train_data_loader, valid_data_loader, test_data_loader)
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, 'classification_report_pheno-focal.txt'), 'wb'))  # save report
        # print(clf_report)
        with open(os.path.join(out_dir, 'Enc-pheno-focal.txt'), 'w') as f:
            f.write(outs)
