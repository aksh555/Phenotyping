...
def get_predictions(model, data_loader):
    ...
    with torch.no_grad():
        for d in data_loader:
            ...
            output = nn.SoftmaxActivation(output)
                # print(output)
                # loss_.backward()
            acts = model.conv1
            #a_k^c
            global_avg_grad = np.mean(model.conv1.weight.grad(), axis=(1,2))
            #L_{Grad-CAM}^c
            for i in range(acts.shape[1]):
                acts[:,i,:] *= global_avg_grad[i]
            heat = nn.relu(np.sum(acts, axis=1))
            print(heat.asnumpy()[0], loss_)
...
def main():
    ...
    df = pd.read_csv(os.path.join(files_dir, 'pheno_rad-ref.csv'))
        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_loss:
                torch.save(model.state_dict(), os.path.join(
                    model_dir, f'radiology-single-note-with-annotations.bin'))
                best_loss = val_loss
                stop_it = 0
            else:
                stop_it += 1
    ...
    if args.do_eval:
        model.load_state_dict(torch.load(
            os.path.join(model_dir, f'radiology-single-note-with-annotations.bin')))
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, 'radiology-single-note-with-annotations.txt'), 'wb'))  # save report
        print(clf_report)
        with open(os.path.join(out_dir, 'radiology-single-note-with-annotations.txt'), 'w') as f:
            f.write(outs)
