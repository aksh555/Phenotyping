...
def get_feats(model, train_data_loader, valid_data_loader, test_data_loader):
    ...
    df_tr.to_csv("./dump_files/train_pheno_disch_feats2.csv")
    df_val.to_csv("./dump_files/valid_pheno_disch_feats2.csv")
    df_test.to_csv("./dump_files/test_pheno_disch_feats2.csv")
...
def main():
    ...
    model = BERT(numClasses, embed_size=512, vocab_size=tz.vocab_size)
        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_loss:
                torch.save(model.state_dict(), os.path.join(
                    model_dir, 'bertattn3_pheno-bce-wa.bin'))
                best_loss = val_loss

    if args.do_eval:
        model.load_state_dict(torch.load(
            os.path.join(model_dir, 'bertattn3_pheno-bce-wa.bin')))
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, 'classification_report_pheno3-bce-wact.txt'), 'wb'))  # save report
        # print(clf_report)
        with open(os.path.join(out_dir, 'bert3-pheno-bce-wact.txt'), 'w') as f:
            f.write(outs)
