        ...
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_loss:
                torch.save(model.state_dict(), os.path.join(
                    model_dir, 'bertattn3_pheno-bce-noact.bin'))
                best_loss = val_loss

    if args.do_eval:
        model.load_state_dict(torch.load(
            os.path.join(model_dir, 'bertattn3_pheno-bce-noact.bin')))
        ...
        pickle.dump(clf_report, open(os.path.join(
            out_dir, 'classification_report_pheno3-bce-wa.txt'), 'wb'))  # save report
        print(clf_report)
        with open(os.path.join(out_dir, 'bert3-pheno-bce-wa.txt'), 'w') as f:
            f.write(outs)
