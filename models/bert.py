    ...
    if args.do_train:
        for epoch in range(EPOCHS):
            ...
            if val_loss < best_loss:
                torch.save(model.state_dict(), os.path.join(model_dir, 'Bert_best-nurse_phys.bin'))
                best_loss = val_loss

    if args.do_eval:
        model.load_state_dict(torch.load(os.path.join(model_dir,'Bert_best-nurse_phys.bin')))
        y_pred, y_test, pred_bools, true_bools = get_predictions(model, test_data_loader)
        ...
        pickle.dump(clf_report, open(os.path.join(out_dir,'classification_report.txt'),'wb')) #save report
        #print(clf_report)
        with open(os.path.join(out_dir, 'Bert-nurse_phys.txt'), 'w') as f:
            f.write(outs)
